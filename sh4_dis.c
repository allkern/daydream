...
char* sh4_disassemble(uint16_t opcode, char* buf, sh4d_state* state) {
    ...
    switch (opcode & 0xf00f) {
        case 0x6003: dst += sprintf(dst, "%-7s R%u, R%u", "mov", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6000: dst += sprintf(dst, "%-7s @R%u, R%u", "mov.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6001: dst += sprintf(dst, "%-7s @R%u, R%u", "mov.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6002: dst += sprintf(dst, "%-7s @R%u, R%u", "mov.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2000: dst += sprintf(dst, "%-7s R%u, @R%u", "mov.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2001: dst += sprintf(dst, "%-7s R%u, @R%u", "mov.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2002: dst += sprintf(dst, "%-7s R%u, @R%u", "mov.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6004: dst += sprintf(dst, "%-7s @R%u+, R%u", "mov.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6005: dst += sprintf(dst, "%-7s @R%u+, R%u", "mov.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6006: dst += sprintf(dst, "%-7s @R%u+, R%u", "mov.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2004: dst += sprintf(dst, "%-7s R%u, @-R%u", "mov.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2005: dst += sprintf(dst, "%-7s R%u, @-R%u", "mov.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2006: dst += sprintf(dst, "%-7s R%u, @-R%u", "mov.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x000c: dst += sprintf(dst, "%-7s @(R0,R%u), R%u", "mov.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x000d: dst += sprintf(dst, "%-7s @(R0,R%u), R%u", "mov.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x000e: dst += sprintf(dst, "%-7s @(R0,R%u), R%u", "mov.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x0004: dst += sprintf(dst, "%-7s R%u, @(R0,R%u)", "mov.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x0005: dst += sprintf(dst, "%-7s R%u, @(R0,R%u)", "mov.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x0006: dst += sprintf(dst, "%-7s R%u, @(R0,R%u)", "mov.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6008: dst += sprintf(dst, "%-7s R%u, R%u", "swap.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6009: dst += sprintf(dst, "%-7s R%u, R%u", "swap.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x200d: dst += sprintf(dst, "%-7s R%u, R%u", "xtrct", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x300c: dst += sprintf(dst, "%-7s R%u, R%u", "add", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x300e: dst += sprintf(dst, "%-7s R%u, R%u", "addc", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x300f: dst += sprintf(dst, "%-7s R%u, R%u", "addv", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3000: dst += sprintf(dst, "%-7s R%u, R%u", "cmp/eq", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3002: dst += sprintf(dst, "%-7s R%u, R%u", "cmp/hs", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3003: dst += sprintf(dst, "%-7s R%u, R%u", "cmp/ge", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3006: dst += sprintf(dst, "%-7s R%u, R%u", "cmp/hi", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3007: dst += sprintf(dst, "%-7s R%u, R%u", "cmp/gt", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x200c: dst += sprintf(dst, "%-7s R%u, R%u", "cmp/str", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2007: dst += sprintf(dst, "%-7s R%u, R%u", "div0s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3004: dst += sprintf(dst, "%-7s R%u, R%u", "div1", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x300d: dst += sprintf(dst, "%-7s R%u, R%u", "dmuls.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3005: dst += sprintf(dst, "%-7s R%u, R%u", "dmulu.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x600e: dst += sprintf(dst, "%-7s R%u, R%u", "exts.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x600f: dst += sprintf(dst, "%-7s R%u, R%u", "exts.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x600c: dst += sprintf(dst, "%-7s R%u, R%u", "extu.b", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x600d: dst += sprintf(dst, "%-7s R%u, R%u", "extu.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x000f: dst += sprintf(dst, "%-7s @R%u+, @R%u+", "mac.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x400f: dst += sprintf(dst, "%-7s @R%u+, @R%u+", "mac.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x0007: dst += sprintf(dst, "%-7s R%u, R%u", "mul.l", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x200f: dst += sprintf(dst, "%-7s R%u, R%u", "muls.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x200e: dst += sprintf(dst, "%-7s R%u, R%u", "mulu.w", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x600b: dst += sprintf(dst, "%-7s R%u, R%u", "neg", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x600a: dst += sprintf(dst, "%-7s R%u, R%u", "negc", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x3008: dst += sprintf(dst, "%-7s R%u, R%u", "sub", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x300a: dst += sprintf(dst, "%-7s R%u, R%u", "subc", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x300b: dst += sprintf(dst, "%-7s R%u, R%u", "subv", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2009: dst += sprintf(dst, "%-7s R%u, R%u", "and", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x6007: dst += sprintf(dst, "%-7s R%u, R%u", "not", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x200b: dst += sprintf(dst, "%-7s R%u, R%u", "or", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x2008: dst += sprintf(dst, "%-7s R%u, R%u", "tst", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x200a: dst += sprintf(dst, "%-7s R%u, R%u", "xor", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x400c: dst += sprintf(dst, "%-7s R%u, R%u", "shad", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x400d: dst += sprintf(dst, "%-7s R%u, R%u", "shld", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf00c: dst += sprintf(dst, "%-7s FR%u, FR%u", "fmov", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf008: dst += sprintf(dst, "%-7s @R%u, FR%u", "fmov.s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf00a: dst += sprintf(dst, "%-7s FR%u, @R%u", "fmov.s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf009: dst += sprintf(dst, "%-7s @R%u+, FR%u", "fmov.s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf00b: dst += sprintf(dst, "%-7s FR%u, @-R%u", "fmov.s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf006: dst += sprintf(dst, "%-7s @(R0,R%u), FR%u", "fmov.s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf007: dst += sprintf(dst, "%-7s FR%u, @(R0,R%u)", "fmov.s", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf000: dst += sprintf(dst, "%-7s FR%u, FR%u", "fadd", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf001: dst += sprintf(dst, "%-7s FR%u, FR%u", "fsub", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf002: dst += sprintf(dst, "%-7s FR%u, FR%u", "fmul", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf00e: dst += sprintf(dst, "%-7s FR0, FR%u, FR%u", "fmac", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf003: dst += sprintf(dst, "%-7s FR%u, FR%u", "fdiv", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf004: dst += sprintf(dst, "%-7s FR%u, FR%u", "fcmp/eq", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0xf005: dst += sprintf(dst, "%-7s FR%u, FR%u", "fcmp/gt", (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
    }
    ...
    switch (opcode & 0xf000) {
        ...
        case 0x5000: dst += sprintf(dst, "%-7s @(%+i,R%u), R%u", "mov.l", (opcode & 0x000f) >> 0, (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
        case 0x1000: dst += sprintf(dst, "%-7s R%u, @(%+i,R%u)", "mov.l", (opcode & 0x000f) >> 0, (opcode & 0x0f00) >> 8, (opcode & 0x00f0) >> 4); return buf;
    ...
    switch (opcode & 0xf08f) {
        case 0x408e: dst += sprintf(dst, "%-7s R%u, R%u_BANK", "ldc", (opcode & 0x0070) >> 4, (opcode & 0x0f00) >> 8); return buf;
        case 0x4087: dst += sprintf(dst, "%-7s @R%u+, R%u_BANK", "ldc.l", (opcode & 0x0070) >> 4, (opcode & 0x0f00) >> 8); return buf;
        case 0x0082: dst += sprintf(dst, "%-7s R%u_BANK, R%u", "stc", (opcode & 0x0f00) >> 8, (opcode & 0x0070) >> 4); return buf;
        case 0x4083: dst += sprintf(dst, "%-7s R%u_BANK, @-R%u", "stc.l", (opcode & 0x0f00) >> 8, (opcode & 0x0070) >> 4); return buf;
    }

    switch (opcode & 0xf11f) {
        case 0xf00c: dst += sprintf(dst, "%-7s DR%u, DR%u", "fmov", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
        ...
        case 0xf000: dst += sprintf(dst, "%-7s DR%u, DR%u", "fadd", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
        case 0xf001: dst += sprintf(dst, "%-7s DR%u, DR%u", "fsub", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
        case 0xf002: dst += sprintf(dst, "%-7s DR%u, DR%u", "fmul", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
        case 0xf003: dst += sprintf(dst, "%-7s DR%u, DR%u", "fdiv", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
        case 0xf004: dst += sprintf(dst, "%-7s DR%u, DR%u", "fcmp/eq", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
        case 0xf005: dst += sprintf(dst, "%-7s DR%u, DR%u", "fcmp/gt", (opcode & 0x0e00) >> 9, (opcode & 0x00e0) >> 5); return buf;
    }

    switch (opcode & 0xf10f) {
        case 0xf008: dst += sprintf(dst, "%-7s @R%u, DR%u", "fmov.d", (opcode & 0x0e00) >> 9, (opcode & 0x00f0) >> 4); return buf;
        case 0xf108: dst += sprintf(dst, "%-7s @R%u, XDn", "fmov.d", (opcode & 0x00f0) >> 4); return buf;
        case 0xf009: dst += sprintf(dst, "%-7s @R%u+, DR%u", "fmov.d", (opcode & 0x0e00) >> 9, (opcode & 0x00f0) >> 4); return buf;
        case 0xf109: dst += sprintf(dst, "%-7s @R%u+, XDn", "fmov.d", (opcode & 0x00f0) >> 4); return buf;
        case 0xf006: dst += sprintf(dst, "%-7s @(R0,R%u), DR%u", "fmov.d", (opcode & 0x0e00) >> 9, (opcode & 0x00f0) >> 4); return buf;
        case 0xf106: dst += sprintf(dst, "%-7s @(R0,R%u), XDn", "fmov.d", (opcode & 0x00f0) >> 4); return buf;
    }

    switch (opcode & 0xf01f) {
        case 0xf00a: dst += sprintf(dst, "%-7s DR%u, @R%u", "fmov.d", (opcode & 0x0f00) >> 8, (opcode & 0x00e0) >> 5); return buf;
        case 0xf01a: dst += sprintf(dst, "%-7s XDm, @R%u", "fmov.d", (opcode & 0x0f00) >> 8); return buf;
        case 0xf00b: dst += sprintf(dst, "%-7s DR%u, @-R%u", "fmov.d", (opcode & 0x0f00) >> 8, (opcode & 0x00e0) >> 5); return buf;
        case 0xf01b: dst += sprintf(dst, "%-7s XDm, @-R%u", "fmov.d", (opcode & 0x0f00) >> 8); return buf;
        case 0xf007: dst += sprintf(dst, "%-7s DR%u, @(R0,R%u)", "fmov.d", (opcode & 0x0f00) >> 8, (opcode & 0x00e0) >> 5); return buf;
